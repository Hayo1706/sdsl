...
import sheetdsl::ui::SheetAppToolbar;
...
alias Model = tuple[str name,
                    ...
                    ParseFunc parseFunc,
                    RunFunc runFunc,
                    bool canRunWithWarnings,
                    bool hasParsed
              ];

App[Model] init(str id,
                start[SDSL] s, 
                ParseFunc parseFunc = set[Message](list[node] n){return {};},
  ...
  = webApp(
      makeApp(
        id,
        Model() { return initModel(id, s,parseFunc, runFunc, canRunWithWarnings);}, 
...
Model initModel(str id, start[SDSL] s, ParseFunc parseFunc,RunFunc runFunc, bool canRunWithWarnings, int rows = 50) {
  ...
  return <id,s,sheet, colTypes, parsedData, parseFunc, runFunc, canRunWithWarnings, false>;
...
CellLoc locOf(Message msg) 
  = /(.*)<row:[0-9]+>,<col:[0-9]+>/ := msg.at.authority 
  ? CellLoc(toInt(row), toInt(col)) : ParseError("Error: <msg> has no location, all errors should have a location set");
...
Model update(parseSheet(), Model model){
  ...
  for (Message err <- errs){
    CellLoc location = locOf(err);
    ErrorType errType = err is warning ? warning() : error();

    tempComments += commentData(location.row, location.col, comment("<err.msg>"), missingCells ? parseerror() : errType);
    model.sheet.sheetData.\data[location.row][location.col] = highlightErrorSubstring(model.parsedData.raw[location.row][location.col], err.at.begin.column,err.at.end.column);
  }
...
void view(Model m) {
  int parseErrors = (0 | it + 1 | commentData(_,_,_, parseerror()) <- m.sheet.comments);
  int errors = (0 | it + 1 | commentData(_,_,_, error()) <- m.sheet.comments);
  int warnings = (0 | it + 1 | commentData(_,_,_, warning()) <- m.sheet.comments);

  bool canParse = !m.hasParsed && parseErrors == 0;
  bool canRun = m.hasParsed && errors == 0;
  canRun = !m.canRunWithWarnings ? canRun && warnings == 0 : canRun;

  div(class("hot-wrapper"),() {
    toolBar(m.name, parseSheet(), runSheet(), canParse, canRun);
    sheetView(m);
  });
}

void sheetView(Model m) {
    spreadsheet(
      m.sheet,
      m.name,
